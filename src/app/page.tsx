...
export default async function HomePage({ searchParams }: { searchParams?: { month?: string } }) {
  ...
  if (selectedMonth && selectedMonth !== "all-time") { // "all-time" check is okay here for data filtering if it somehow gets in URL
    displayBets = allBets.filter(bet => {
      const betDate = new Date(bet.createdAt);
      const createdInMonth = `${betDate.getFullYear()}-${String(betDate.getMonth() + 1).padStart(2, '0')}` === selectedMonth;
      let resolvedInMonth = false;
      if (bet.resolvedAt) {
        const resolvedDate = new Date(bet.resolvedAt);
        resolvedInMonth = `${resolvedDate.getFullYear()}-${String(resolvedDate.getMonth() + 1).padStart(2, '0')}` === selectedMonth;
      }
      return createdInMonth || resolvedInMonth;
    });
  }
  
  const activeBets = displayBets.filter(bet => bet.status === 'Pending').sort((a,b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime());
  const resolvedBetsSorted = displayBets.filter(bet => bet.status === 'Won' || bet.status === 'Lost' || bet.status === 'Void' || bet.status === 'CashedOut').sort((a,b) => new Date(b.resolvedAt || b.createdAt).getTime() - new Date(a.resolvedAt || a.createdAt).getTime());
    ...
    <div className="space-y-8">
      ...
      <Tabs defaultValue="active" className="w-full">
        <TabsList className="grid w-full grid-cols-2 mb-6 sm:w-auto sm:mx-auto">
          <TabsTrigger value="active">Activas ({activeBets.length})</TabsTrigger>
          <TabsTrigger value="resolved">Resueltas ({resolvedBetsSorted.length})</TabsTrigger>
        </TabsList>
