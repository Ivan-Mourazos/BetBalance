...
export async function calculateOverallStats(bets: Bet[], transactions: Transaction[], selectedMonth?: string): Promise<OverallStats> {
  ...
  if (selectedMonth && selectedMonth !== "all-time") {
    filteredBets = bets.filter(bet => {
      ...
      const createdInSelectedMonth = `${betDate.getFullYear()}-${String(betDate.getMonth() + 1).padStart(2, '0')}` === selectedMonth;
      let resolvedInSelectedMonth = false;
      if (bet.resolvedAt) {
        const resolvedDate = new Date(bet.resolvedAt);
        resolvedInSelectedMonth = `${resolvedDate.getFullYear()}-${String(resolvedDate.getMonth() + 1).padStart(2, '0')}` === selectedMonth;
      }
    ...
    filteredTransactions = transactions.filter(t => {
      ...
      return `${tDate.getFullYear()}-${String(tDate.getMonth() + 1).padStart(2, '0')}` === selectedMonth;
    });
  }
  ...
  const financialBalance = totalWithdrawals - totalDeposits - totalPicksPurchases;

  // For betting stats, consider only bets relevant to the period (created or resolved in month)
  const resolvedBets = filteredBets.filter(b => b.status === 'Won' || b.status === 'Lost' || b.status === 'Void' || b.status === 'CashedOut');
  ...
  const betsCashedOut = resolvedBets.filter(b => b.status === 'CashedOut').length;
  ...
  const totalReturnsFromBets = resolvedBets.reduce((sum, b) => sum + (b.actualWinnings ?? 0), 0);
  ...
  const profitPercentage = wageredExcludingVoidAndCashout > 0 ? (profitFromWonLostBets / wageredExcludingVoidAndCashout) * 100 : 0;
